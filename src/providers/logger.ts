...
import chalk from 'chalk'; // used for colorizing console output
import moment from 'moment'; // used for calculating response times
...
interface CustomConsoleTransportOptions
...
const consoleFormatter = (logEntry: LogEntry): LogEntry => {
  ...
  switch (logEntry.label) {
    ...
    case 'express_response':
      ...
      const resp_str = color(
        `${logEntry.req.method} ${logEntry.req.url} (${logEntry.status}) `
      );
      Object.assign(logEntry, {
        [MESSAGE]:
          `${moment().toISOString()} ` +
  ...
  transports: [
    ...
    new winston.transports.Console({
      ...
      format: winston.format.combine(
        // filterTransport({ target: 'console' }),
    ...
    new winston.transports.File({
      ...
      maxsize: logs.maxFileSize,
      zippedArchive: logs.zipOldLogs
...
// Initialize request
const initRequest = function initRequest(
  req: any,
  _res: any,
  next: NextFunction
) {
...
const logResponse = (req: any, res: any, next: NextFunction) => {
  ...
  function responseLog() {
    if (res.statusCode !== 401) {
      ...
      res.responseTime = (
        (res.timeDiff[0] * 1e9 + res.timeDiff[1]) *
        1e-6
      ).toFixed(3);
      // log the response
