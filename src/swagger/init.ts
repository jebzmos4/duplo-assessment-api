...
export default async function main(): Promise<number> {
  try {
    ...
    console.log(
      '\x1b[32m%s\x1b[0m',
      `Valid OpenAPI spec in file ${apiFile}: Valid`
    );
    return 0;
  } catch (err) {
    fs.writeFileSync('openapi_err.txt', '');

    // log failure and return non-zero value
    console.log(
      'There is a problem with the OpenAPI spec in the code, error file written to openapi_err.txt'
    );
    return 1;
  }
}
